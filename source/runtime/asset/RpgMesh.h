...
public:
	RpgMesh(const RpgStringID& in_Name) noexcept;
...
public:
	...
	// Write lock vertex position. Must call VertexPositionWriteUnlock_Position after done writing!
	inline RpgVertexMeshPositionArray& VertexWriteLock_Position() noexcept
	{
		AcquireSRWLockExclusive(&LockPosition);
		return Positions;
	}

	// Unlock write vertex position
	inline void VertexWriteUnlock_Position() noexcept
	{
		ReleaseSRWLockExclusive(&LockPosition);
	}


	// Write lock vertex normal-tangent. Must call VertexPositionWriteUnlock_NormalTangent after done writing!
	inline RpgVertexMeshNormalTangentArray& VertexWriteLock_NormalTangent() noexcept
	{
		AcquireSRWLockExclusive(&LockNormalTangent);
		return NormalTangents;
	}

	// Unlock write vertex normal-tangent
	inline void VertexWriteUnlock_NormalTangent() noexcept
	{
		ReleaseSRWLockExclusive(&LockNormalTangent);
	}


	// Write lock vertex texcoord. Must call VertexPositionWriteUnlock_TexCoords after done writing!
	inline RpgVertexMeshTexCoordArray& VertexWriteLock_TexCoord() noexcept
	{
		AcquireSRWLockExclusive(&LockTexCoord);
		return TexCoords;
	}

	// Unlock write vertex texcoords
	inline void VertexWriteUnlock_TexCoord() noexcept
	{
		ReleaseSRWLockExclusive(&LockTexCoord);
	}


	// Write lock vertex index. Must call VertexPositionWriteUnlock_Index after done writing!
	inline RpgVertexIndexArray& VertexWriteLock_Index() noexcept
	{
	...
	// Unlock write vertex texcoords
	inline void VertexWriteUnlock_Index() noexcept
	{
