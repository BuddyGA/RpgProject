...
enum class RpgMaterialColorBlendMode : uint8_t
{
	NONE = 0,
	ADDITIVE,
	OPACITY_MASK,
...
struct RpgMaterialPipelineState
{
	...
	RpgMaterialColorBlendMode BlendMode{ RpgMaterialColorBlendMode::NONE };
...
public:
	...
	inline void SetVectorValue(const RpgName& paramName, RpgVector4 in_Value) noexcept
	{
		...
		RPG_CheckV(index != RPG_INDEX_INVALID, "Vector param %s not found!", *paramName.ToString());
	...
	inline void SetScalarValue(const RpgName& paramName, float in_Value) noexcept
	{
		...
		RPG_CheckV(index != RPG_INDEX_INVALID, "Scalar param %s not found!", *paramName.ToString());
...
public:
	RpgMaterial(const RpgStringID& in_Name) noexcept;
	RpgMaterial(const RpgStringID& in_Name, const RpgMaterialPipelineState& in_PipelineState, const RpgMaterialParameterLayout& in_ParameterLayout) noexcept;
	RpgMaterial(const RpgStringID& in_Name, const RpgSharedMaterial& in_ParentMaterial) noexcept;
...
public:
	...
	inline const RpgMaterialPipelineState& GetRenderState() const noexcept
	{
	...
	{
		return PipelineState.BlendMode == RpgMaterialColorBlendMode::TRANSPARENCY;
	}
...
public:
	[[nodiscard]] static RpgSharedMaterial CreateShared(const RpgStringID& name, const RpgMaterialPipelineState& renderState, const RpgMaterialParameterLayout& parameterLayout = RpgMaterialParameterLayout()) noexcept;
	[[nodiscard]] static RpgSharedMaterial CreateSharedInstance(const RpgStringID& name, const RpgSharedMaterial& parentMaterial) noexcept;
