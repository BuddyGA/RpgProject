...
RpgTexture2D::RpgTexture2D(const RpgStringID& in_Name) noexcept
...
RpgTexture2D::RpgTexture2D(const RpgStringID& in_Name, RpgTextureFormat::EType in_Format, uint16_t in_Width, uint16_t in_Height, uint8_t in_MipCount) noexcept
...
{
	RPG_LogDebug(RpgLogTexture, "Destroy texture (%s)", *GetAssetName().ToString());
...
void RpgTexture2D::GPU_UpdateResource() noexcept
{
	...
	if (bShouldCreateNew)
	{
		...
		RPG_D3D12_SetDebugNameAllocation(GpuAlloc, "RES_%s", *GetAssetName().ToString());
	}
}
...
void RpgTexture2D::InitializeMips() noexcept
{
	...
	RPG_D3D12_SetDebugNameAllocation(PixelStagingBuffer, "STG_%s", *GetAssetName().ToString());
...
RpgTextureRenderTarget::RpgTextureRenderTarget(const RpgStringID& in_Name, RpgTextureFormat::EType in_Format, uint16_t in_Width, uint16_t in_Height) noexcept
	: RpgTexture2D(in_Name)
{
...
RpgTextureDepthStencil::RpgTextureDepthStencil(const RpgStringID& in_Name, RpgTextureFormat::EType in_Format, uint16_t in_Width, uint16_t in_Height) noexcept
	: RpgTexture2D(in_Name)
{
...
RpgTextureDepthCube::RpgTextureDepthCube(const RpgStringID& in_Name, RpgTextureFormat::EType in_Format, uint16_t in_Width, uint16_t in_Height) noexcept
