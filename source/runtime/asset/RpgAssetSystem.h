...
public:
	...
	inline void RegisterAssetClass() noexcept
	{
		...
		const RpgStringID className = defaultObject->GetAssetClassName();
	...
	RpgSharedAsset LoadAsset(const RpgStringID& assetPath) noexcept;

	// Load asset synchronously
	// @param path - Asset path relative to asset directory
	// @return SharedPtr to asset if path is valid and asset is loaded
	template<typename TAsset>
	inline RpgSharedPtr<TAsset> LoadAsset(const RpgStringID& assetPath) noexcept
	{
		...
		if (asset)
		{
			RPG_Check(asset->GetAssetPath() == assetPath);
	...
	RpgSharedAsset LoadAssetAsync(const RpgStringID& assetPath) noexcept;
	...
	template<typename TAsset>
	inline RpgSharedPtr<TAsset> LoadAssetAsync(const RpgStringID& assetPath)
	{
	...
	inline bool IsAssetRegistered(const RpgStringID& assetPath, RpgAssetInfo* optOut_AssetInfo = nullptr) const noexcept
	{
		...
		RPG_Check(assetInfo->Path == assetPath);
	...
	inline bool IsAssetLoaded(const RpgStringID& assetPath) const noexcept
	{
...
private:
	...
	void RegisterAssetFile(const RpgFilePath& filePath, RpgStringID* optOut_AssetPath = nullptr) noexcept;


	// Get asset path 
	// @param filePath - Absolute path to an asset file
	// @return Relative asset path
	RpgStringID GetAssetPathFromFile(const RpgFilePath& filePath) const noexcept;


	inline void CreateAsset(RpgSharedAsset& out_Asset, const RpgStringID& className, const RpgStringID& assetPath) const noexcept
	{
	...
	inline RpgSharedAsset FindLoadedAsset(const RpgStringID& assetPath) const noexcept
	{
	...
	inline void AddLoadedAsset(const RpgStringID& assetPath, const RpgSharedAsset& asset) noexcept
	{
		RPG_PLATFORM_ScopedLock(LoadedMutex);
...
private:
	...
	struct FRegistry
	{
		...
		RpgMap<RpgStringID, const RpgAssetObject*, 8> DefaultObjectTable;

		// Registered assets
		RpgMap<RpgStringID, RpgAssetInfo, 8> AssetInfoTable;
	};
	FRegistry Registry;


	// Loading assets
	RpgMap<RpgStringID, RpgSharedAsset, 8> LoadingAssetTable;
	RpgArray<RpgAssetTask_Loader*> LoadingAssetTasks;
	mutable RpgPlatformMutex LoadingMutex;

	// Loaded assets
	RpgMap<RpgStringID, RpgSharedAsset, 8> LoadedAssetTable;
