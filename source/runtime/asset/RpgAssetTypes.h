...
typedef RpgSet<RpgStringID, 8> RpgAssetReferences;
...
public:
	RpgAssetObject(const RpgStringID& in_Name) noexcept
		: Name(in_Name)
	{
	}
	...
	virtual RpgStringID GetAssetClassName() const noexcept
	{
	...
	inline const RpgStringID& GetAssetName() const noexcept
	{
		return Name;
	}

	inline const RpgStringID& GetAssetPath() const noexcept
	{
		return Path;
	}
...
public:																										\
	virtual RpgStringID GetAssetClassName() const noexcept override { return #classType; }					\
