...
RpgFont::RpgFont(const RpgStringID& in_Name, const RpgString& ttfFilePath, float in_SizePx, int in_UnicodeStart, int in_UnicodeCount) noexcept
{
	RPG_LogDebug(RpgLogFont, "Create font (%s)", *in_Name.ToString());
	...
	Texture = RpgPointer::MakeShared<RpgTexture2D>(RpgStringID::Format("tex2d_%s", *Name.ToString()), RpgTextureFormat::TEX_2D_R, textureDimension.X, textureDimension.Y, 1);
	{
...
{
	RPG_LogDebug(RpgLogFont, "Destroy font (%s)", *Name.ToString());
...
RpgSharedFont RpgFont::CreateShared(const RpgStringID& name, const RpgString& ttfFilePath, float sizePx, int unicodeStart, int unicodeCount) noexcept
{
