...
namespace RpgAssetStream
{
	static RpgFilePath Write(RpgAssetObject* asset, const char* dstFolder) noexcept
	{
		...
		// output file path
		const RpgFilePath filePath = RpgString::Format("%s%s%s", *folderPath, *asset->GetAssetName().ToString(), fileExt);
		...
		RpgAssetFileHeader fileHeader;
		{
			...
			fileHeader.AssetClassName = *asset->GetAssetClassName().ToString();
		}
	...
	static RpgBinaryStreamReader CreateReaderForAsset(RpgAssetObject* asset) noexcept
	{
		...
		const RpgStringID& path = asset->GetAssetPath();
...
bool RpgAssetSystem::IsValidAssetFile(const RpgFilePath& filePath, RpgAssetInfo* optOut_AssetInfo) noexcept
{
	...
	if (optOut_AssetInfo)
	{
		optOut_AssetInfo->Class = RpgStringID(header.AssetClassName);
...
RpgSharedAsset RpgAssetSystem::LoadAsset(const RpgStringID& assetPath) noexcept
{
	RPG_IsMainThread();

	if (assetPath.IsEmpty())
	{
		RPG_CONSOLE_Warn(RpgLogAsset, "Fail to load asset. Invalid asset path (%s)!", *assetPath.ToString());
	...
	{
		RPG_CONSOLE_Error(RpgLogAsset, "Fail to load asset (%s). Asset not found in registry!", *assetPath.ToString());
		return RpgSharedAsset();
	}
	...
	// add to loaded data
	AddLoadedAsset(assetPath, asset);
	RPG_CONSOLE_Log(RpgLogAsset, "Loaded asset (%s)", *assetPath.ToString());

	return asset;
}


RpgSharedAsset RpgAssetSystem::LoadAssetAsync(const RpgStringID& assetPath) noexcept
{
	//RPG_IsMainThread();
	...
	// check if loading
	{
		...
		RPG_Log(RpgLogAsset, "Loading asset (%s)", *assetPath.ToString());
	...
	for (const RpgStringID& ref : assetRefs)
	{
		LoadAssetAsync(ref);
	}
...
void RpgAssetSystem::RegisterAssetFile(const RpgFilePath& filePath, RpgStringID* optOut_AssetPath) noexcept
{
	// get asset path
	const RpgStringID assetPath = GetAssetPathFromFile(filePath);
	...
	RPG_CONSOLE_Log(RpgLogAsset, "Added asset to registry (Path: %s, Type: %s)", *assetPath.ToString(), RPG_ASSET_FILE_TYPE_NAMES[static_cast<uint16_t>(assetInfo.Type)]);
}


RpgStringID RpgAssetSystem::GetAssetPathFromFile(const RpgFilePath& filePath) const noexcept
{
