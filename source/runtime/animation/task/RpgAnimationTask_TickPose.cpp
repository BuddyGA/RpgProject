...
void RpgAnimationTask_TickPose::Execute() noexcept
{
	for (int i = 0; i < AnimationComponents.GetCount(); ++i)
	{
		...
		if (!comp->Skeleton)
		{
			RPG_LogWarn(RpgLogAnimation, "Fail to update animation for gameobject (%s). Invalid skeleton!", *comp->GameObject.GetName().ToString());
			continue;
		}
		...
		if (!comp->Clip)
		{
			RPG_LogWarn(RpgLogAnimation, "Fail to update animation for gameobject (%s). Invalid animation clip!", *comp->GameObject.GetName().ToString());
			continue;
		}
