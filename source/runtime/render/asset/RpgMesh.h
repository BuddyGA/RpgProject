...
public:
	...
	// Write lock vertex position. Must call VertexWritePositionUnlock after done writing!
	inline RpgVertexMeshPositionArray& VertexWritePositionLock() noexcept
	{
		AcquireSRWLockExclusive(&LockPosition);
		return Positions;
	}

	// Write unlock vertex position
	inline void VertexWritePositionUnlock() noexcept
	{
		ReleaseSRWLockExclusive(&LockPosition);
	}


	// Write lock vertex normal-tangent. Must call VertexWriteNormalTangentUnlock after done writing!
	inline RpgVertexMeshNormalTangentArray& VertexWriteNormalTangentLock() noexcept
	{
		AcquireSRWLockExclusive(&LockNormalTangent);
		return NormalTangents;
	}

	// Write unlock vertex normal-tangent
	inline void VertexWriteNormalTangentUnlock() noexcept
	{
		ReleaseSRWLockExclusive(&LockNormalTangent);
	}


	// Write lock vertex texcoord. Must call VertexWriteTexCoordUnlock after done writing!
	inline RpgVertexMeshTexCoordArray& VertexWriteTexCoordLock() noexcept
	{
		AcquireSRWLockExclusive(&LockTexCoord);
		return TexCoords;
	}

	// Write unlock vertex texcoord
	inline void VertexWriteTexCoordUnlock() noexcept
	{
		ReleaseSRWLockExclusive(&LockTexCoord);
	}


	// Write lock vertex index. Must call VertexWriteIndexUnlock after done writing!
	inline RpgVertexIndexArray& VertexWriteIndexLock() noexcept
	{
		AcquireSRWLockExclusive(&LockIndex);
		return Indices;
	}

	// Write unlock vertex index
	inline void VertexWriteIndexUnlock() noexcept
	{
