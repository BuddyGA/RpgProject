...
{
	RpgSharedEditorModel model = RpgPointer::MakeShared<RpgEditorModel>("MDL_DEF_floor");
	{
		model->AddLod();
		model->AddMeshEmpty();
...
static void TestLevel_AddCube(RpgWorld* world, const RpgTransform& transform) noexcept
{
	static int Counter = 0;
	...
	RpgGameObject box = world->CreateGameObject(RpgName::Format("test_cube_%i", Counter++));
	{
		...
		meshComp.Mesh = g_AssetSystem->LoadAsset<RpgMesh>(RpgStringID("game/mesh/cube_x100_y100_z100"));
...
static void TestLevel_Import(RpgWorld* world, const RpgFilePath& sourceFilePath, float scale = 1.0f, bool bGenerateTextureMipMaps = false, bool bIgnoreTextureNormals = false) noexcept
{
	...
	{
		const RpgSharedEditorModel& model = importedModels[i];
...
static bool TestLevel_PrimitiveShapes(RpgWorld* world) noexcept
{
	...
	// test box center
	{
		transform.Position = RpgVector3(0.0f, 8.0f + 64.0f, 200.0f);
...
bool RpgTest::Game::Create(RpgWorld* world) noexcept
{
	...
	//TestLevel_Import(world, RpgFileSystem::GetAssetRawDirPath() + "model/RiggedFigure.glb", 100.0f);
	//TestLevel_Import(world, RpgFileSystem::GetAssetRawDirPath() + "model/CesiumMilkTruck.glb", 100.0f);
